...
import { readableStreamToString, PaychexError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new PaychexError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new PaychexError(e, e.response?.data)
                }
                if (innerError instanceof PaychexError) {
                    // Got "PaychexError" from the above try block
                    throw innerError;
                }
